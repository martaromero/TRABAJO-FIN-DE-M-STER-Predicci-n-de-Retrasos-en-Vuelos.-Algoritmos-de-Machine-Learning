...
rf = RandomForestClassifier(labelCol="label", featuresCol="features", numTrees=10, maxDepth=16) 

# Fit the model
model = rf.fit(train_LP)

#Save the model
#model.save("dbfs:/dataset/modelo_binario_RF")
...
# sameModel = model.load("dbfs:/dataset/modelo_binario_RF")
...
if ppv==0: dp=0;
else: dp=3*(sensibilidad*npv*ppv) /((sensibilidad*ppv) + (sensibilidad*npv) + 
